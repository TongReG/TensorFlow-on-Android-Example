...
public class Camera2Utils extends Fragment
            ...
            if (mImage != null) {
                ...
                switch (mImageFormat) {
                    case ImageFormat.JPEG:
                        Log.i(TAG, "mImageFormat = JPEG");
                        mImageBytes = ImageUtils.getImageByte(mImage, ImageUtils.NV21);
                        break;
                    case ImageFormat.YUV_420_888:
                        Log.i(TAG, "mImageFormat = YUV420");
                        mImageBytes = ImageUtils.getImageByte(mImage, ImageUtils.YUV420P);
                        break;
                    case ImageFormat.YUV_444_888:
                        Log.i(TAG, "mImageFormat = YUV444");
                        mImageBytes = null;
                        break;
        ...
        for (Size option : choices) {
            if (option.getWidth() <= maxWidth && option.getHeight() <= maxHeight) {
                if (option.getWidth() >= textureViewWidth &&
                    ...
                    Log.i(TAG, "Support in bigEnough width: " + option.getWidth() + " height: " + option.getHeight());
                } else {
                    notBigEnough.add(option);
                    Log.i(TAG, "Support in notbigEnough width: " + option.getWidth() + " height: " + option.getHeight());
                }
            }
        }
    ...
    public static Camera2Utils newInstance() {
        return new Camera2Utils();
    }
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                /*
                现在很多推出的设备虽然都是Android5.0以上的，也就是支持Camera2的API，
                虽然它们实现了这些API，但是实际上可配置的参数比Camera少，甚至性能都比Camera差，为此厂商在系统中写明了对Camera2的支持程度
                得到的值中有这三种：LIMITED值为0，FULL值为1，LEGACY值为2，其支持程度为FULL > LIMITED > LEGACY
                关于LEGACY是这样说的：
                遗留的设备不支持每帧控制、手动传感器控制、手动后处理、任意裁剪区域，并且具有宽松的性能约束
                可以说兼容性很差了，我在自己手机上验证了下，发现LEGACY，属于遗留机！
                可以预见很多手机上并不完全支持Camera2API，所以如果手机并不是FULL支持的话，建议还是使用Camera1
                */

                Integer hardwareLevel = characteristics.get(CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL);
                Log.i(TAG, "Camera Hardware Level = " + hardwareLevel + " (LIMITED=0 FULL=1 LEGACY=2 LEVEL3=3)");
                ...
                Log.i(TAG, "WindowManager width: " + maxPreviewWidth + " height: " + maxPreviewHeight);
                ...
                Log.i(TAG, "TextureView width: " + rotatedPreviewWidth + " height: " + rotatedPreviewHeight);
                Log.i(TAG, "View Finally width: " + mPreviewSize.getWidth() + " height: " + mPreviewSize.getHeight());
            ...
            ErrorDialog.newInstance(getString(R.string.Camera2Error))
                    .show(getChildFragmentManager(), FRAGMENT_DIALOG);
        }
    }
            ...
            mCameraDevice.createCaptureSession(Arrays.asList(surface, mImageReader.getSurface()),
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigureFailed(
                            ...
                            showToast("Create CameraCaptureSession Failed");
                        }
                    }, null
            );
        } catch (CameraAccessException e) {
